...
if __name__ == '__main__':
    transformations = transforms.Compose([transforms.ToPILImage(),transforms.Grayscale(num_output_channels=1),transforms.Resize(size=64),transforms.ToTensor(),transforms.Normalize(mean=[0.5937], std=[0.0810])])
    ...
    if args.accelerator == "ddp":
        rddp = True
    trainer = pl.Trainer(max_epochs=args.epochs, replace_sampler_ddp=rddp, gpus=args.gpus,logger=tb_logger,num_nodes=args.nodes,accelerator=args.accelerator,auto_lr_find=False,benchmark=True,fast_dev_run=False,gradient_clip_val=0,sync_batchnorm=True) #flush_logs_every_n_steps=1
