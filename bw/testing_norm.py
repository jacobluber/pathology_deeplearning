...
if __name__ == '__main__':
    transformations = transforms.Compose([transforms.ToPILImage(mode="RGB"),transforms.CenterCrop(size=64),transforms.ToTensor(),transforms.Normalize(mean=(0.6499, 0.5505, 0.6602),std=(0.0745, 0.0878, 0.0686)),transforms.Grayscale(num_output_channels=1)])
    #transformations = transforms.Compose([transforms.ToPILImage(),transforms.ToTensor(),transforms.Normalize(mean=[0.6499, 0.5505, 0.6602],std=[0.0745, 0.0878, 0.0686]),transforms.ToPILImage,transforms.CenterCrop(64),transforms.Grayscale(num_output_channels=1),transforms.ToTensor()])
